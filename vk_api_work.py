...
class VKBot:
    ...
    def write_some_msg(self, user_id, some_msg, keyboard=None):
        post = {"user_id": user_id, "message": some_msg, "random_id": 0}
    ...
    def send_photo(self, user_id, photo_id):
        self.vk.method(
            "messages.send",
            {"user_id": user_id, "attachment": "photo" + photo_id, "random_id": 0},
        )

    def send_profile_info(self, user_id):
        info = self.vk.method(
            "users.get", {"user_ids": user_id, "fields": "bdate, sex, city, photo_id"}
        )
    ...
    def pair_up(self, user_id, keyboard):
        ...
        while True:
            ...
            if persons["items"][0]["is_closed"] is True:
                ...
                photos_list = dict(
                    sorted(photos_dict.items(), key=lambda item: item[1])
                )
            ...
            if event.type == VkEventType.MESSAGE_NEW:
                if event.to_me:
                    ...
                    if request == "start":
                        ...
                        message = "\n".join(
                            "{} {}".format(key, val) for key, val in persons.items()
                        )
